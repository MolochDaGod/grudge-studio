...
export default function EngineerPage() {
  ...
  const tabs: { value: TabValue; label: string }[] = [
    { value: "tree", label: "Schematic Tree" },
    { value: "craft", label: "Assembly" },
    { value: "upgrade", label: "Retrofit" },
    ...
    <div className="flex flex-col h-full min-h-0">
      <header className="flex-shrink-0 flex flex-col md:flex-row justify-between items-center gap-3 px-4 py-2 border-b-2 border-[hsl(43_50%_30%)] bg-gradient-to-b from-[hsl(225_25%_16%)] to-[hsl(225_28%_12%)]">
        <div className="flex items-center gap-3">
          <img src={engineerIcon} alt="Engineer" className="w-10 h-10 md:w-12 md:h-12 rounded-lg" />
          <div>
            <h1 className="text-xl md:text-2xl font-uncial text-orange-500 leading-none">Grand Machinist</h1>
            <p className="text-[10px] text-slate-500 uppercase tracking-[0.15em] mt-0.5">Engineer Specialization</p>
          </div>
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-hidden">
        {activeTab === "tree" && (
          <TreeVisualizer 
            nodes={engineerData.treeData} 
            color={engineerData.color} 
            bgImage={engineerData.bgImage} 
            profession="Engineer" 
            fullscreen 
          />
        )}
        {activeTab === "craft" && (
          <div className="h-full overflow-auto p-4">
            <CraftingInterface data={engineerData} />
          </div>
        )}
        {activeTab === "upgrade" && (
          <div className="h-full overflow-auto p-4">
            <UpgradeInterface data={engineerData} />
          </div>
        )}
        {activeTab === "activities" && (
          <div className="h-full overflow-auto p-4">
            <ActivitiesPanel profession="engineer" accentColor="orange" />
          </div>
        )}
      </div>
    </div>
